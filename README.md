...
2. **😴 안 하면 잠듭니다** — 단순히 "집중할 때 활성 / 쉴 때 잠듦"이 아니라, **오늘 한 번도 집중하지 않으면 캐릭터가 흑백으로 잠듭니다.** 듀오링고 스트릭처럼 *지키고 싶은 것을 잃지 않으려는* 동기로 설계했습니다.
...
| 캐릭터 (깨어남) | 친구 | 통계 · 잔디 | 설정 |
|:---:|:---:|:---:|:---:|
| <img src="docs/screenshots/ko/ko_main.png" width="220" alt="메인"> | <img src="docs/screenshots/ko/ko_friends.png" width="220" alt="친구"> | <img src="docs/screenshots/ko/ko_stats.png" width="220" alt="통계"> | <img src="docs/screenshots/ko/ko_settings.png" width="220" alt="설정"> |

### 🖥️ 메뉴바 모습

실행하면 맥 상단 메뉴바에 이렇게 떠 있습니다.

| 오늘 집중함 (깨어남 · 연속일) | 집중·휴식 중 (남은 시간) |
|:---:|:---:|
| <img src="docs/screenshots/bar/wakeup.png" height="36" alt="깨어난 펫과 연속일"> | <img src="docs/screenshots/bar/minutes.png" height="36" alt="남은 시간"> |

> 올린 캐릭터가 그대로 메뉴바에 뜹니다. 오늘 한 번도 집중 안 하면 흑백으로 잠들어 있다가, 집중하면 컬러로 깨어나고 옆에 연속일이 붙습니다. 집중·휴식 중엔 남은 시간이 표시돼요.
...
- **👀 올리기 전에 미리보기** — 도트와 메뉴바 크기로 어떻게 보이는지 먼저 확인. 배경이 있는 사진은 **배경을 자동으로 지울 수 있어요**
- **🔥 연속 기록(스트릭)** — 매일 집중하면 연속일이 쌓이고, 하루 걸르면 리셋
- **😴 깨어남/잠듦** — 오늘 집중하면 캐릭터가 컬러로 깨어나 통통 튐 / 안 하면 흑백으로 잠듦
- **🏅 마일스톤 아우라** — 연속 3일 BRONZE → 7일 SILVER → 30일 GOLD → 100일 DIAMOND 테두리
- **🌱 활동 히트맵** — 월 달력으로 공부 기록을 한눈에 (월 이동 · 날짜 hover로 그날 상세)
- **🍅 단순한 포모도로** — 집중 / 휴식 두 가지만 정하면 끝. 목표를 세울 필요가 없습니다
...
- **▶️ 작업 앱 감지** — 정해둔 앱(Xcode, VS Code 등)을 켜면 3초 뒤 집중이 자동으로 시작 (거절할 시간을 줍니다). 그 앱에서 계속 작업 중일 때도 알아챕니다
...
1. 메뉴바 아이콘 클릭 → **키울 캐릭터 이미지 선택**
   - 아무 사진이나 올려도 됩니다. 고르면 **도트와 메뉴바 크기로 미리 보여주고**, 배경이 있는 사진은 배경을 지울지 물어봅니다.
   - 투명 배경 PNG(누끼)라면 손댈 게 없어 가장 또렷합니다.
2. **집중 시작** → 그 순간부터 캐릭터가 깨어납니다
3. 오늘 집중한 기록이 남으면 **연속일**이 이어집니다 🔥 (중단해도 5분 이상 했으면 시간은 남습니다)
...
- **Vision** — 업로드 사진의 배경 제거. macOS 에 들어 있어 앱 용량이 늘지 않고 기기 안에서만 처리됩니다
...
│               배경 제거 (BackgroundRemover)
└─ Views/       화면 (Popover, Stats/Settings, Friends, 업로드 미리보기, 제안 카드)
